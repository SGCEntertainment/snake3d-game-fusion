...
public class MazeGenerator : NetworkBehaviour 
{
    ...
    //public static MazeGenerator Instance => Singleton<MazeGenerator>.Instance;
    #region Variables:
    // ------------------------------------------------------
    ...
    public  NetworkObject cellPrefab;
    ...
    public void RPC_UpdateMazeWalls(WallsNetworkStruct[] walls)
    {
        foreach(WallsNetworkStruct wallsNetworkStruct in walls)
        {
            WallsStructs.Add(wallsNetworkStruct);
        }
    }

    public static void OnMazeWallsChanged(Changed<MazeGenerator> changed)
    {
        changed.Behaviour.UpdateLocalWalls();
    }

    public void UpdateLocalWalls()
    {
        CellScript[] sortedCellScripts = this.GetMazeCells();

        for(int i = 0; i < sortedCellScripts.Length; i++)
        {
            WallsNetworkStruct wallsNetworkStruct = WallsStructs[i];

            sortedCellScripts[i].wallL.SetActive(wallsNetworkStruct.setActiveLeft);
            sortedCellScripts[i].wallR.SetActive(wallsNetworkStruct.setActiveRight);
            sortedCellScripts[i].wallD.SetActive(wallsNetworkStruct.setActiveDown);
            sortedCellScripts[i].wallU.SetActive(wallsNetworkStruct.setActiveUp);
    ...
    void SetWallsStruct()
    {
        ...
        {
            CellScript cellScript = t.GetComponent<CellScript>();
            WallsNetworkStruct wallsNetworkStruct = new WallsNetworkStruct
            {
                setActiveLeft = cellScript.wallL.activeSelf,
                setActiveRight = cellScript.wallR.activeSelf,
                setActiveDown = cellScript.wallD.activeSelf,
                setActiveUp = cellScript.wallU.activeSelf,
            };

            wallsNetworkStructs.Add(wallsNetworkStruct);
        }

        RPC_UpdateMazeWalls(wallsNetworkStructs.ToArray());
    }

    public void ResetPlayerPosition(NetworkTransform playerTransform)
    {
        playerTransform.TeleportToPosition(spawnPosition);
    }
    ...
    public void GenerateCell(Vector2 pos, Vector2 keyPos)
    {
        ...
        newCell.cellObject = Runner.Spawn(cellPrefab, pos, cellPrefab.transform.rotation);


        //newCell.cellObject.transform.parent = transform;
        newCell.cellObject.transform.parent = MazeParent;
    ...
    public class Cell
    {
        ...
        public NetworkObject cellObject;
